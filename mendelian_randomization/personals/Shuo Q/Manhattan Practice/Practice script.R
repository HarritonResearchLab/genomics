...
library(qqman)
library(data.table)
setwd("D:/Software/R-4.1.3/Projects/Mendelian Randomization/material")
...
bmi_exa_hm$chr <- as.numeric(bmi_exa_hm$chr)
bmi_exa_hm$pos <- as.numeric(bmi_exa_hm$pos)
...
manhattan(
  bmi_exa_hm,
  snp = "SNP",
   # this section only accept the variables as numebrs, so make sure to convert those into numbers.
  chr = "chr",
  bp = "pos",
  p = "pval.outcome",
  # --------------------------------
